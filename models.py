...
class SwinTransformerModelTL(nn.Module):
    def __init__(self, input_channels, max_essay_length, kernel_size, stride, pretrained_model):
        super().__init__()
        ...
        self.pretrained_model.features[0][0] = nn.Conv2d(input_channels, 96, kernel_size=(1,kernel_size), stride=stride)
        self.pretrained_model.head = nn.Linear(in_features=768, out_features=1, bias=True)
        self.fc_essay_set = nn.Linear(8, max_essay_length)
